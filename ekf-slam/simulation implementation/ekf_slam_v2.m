classdef ekf_slam_v2 < handle
    %EKF_SLAM The EKF algorithm for SLAM
    
    properties
        x = zeros(3,1); % The estimated state vector
        P = 0.0001*ones(3,3); % The estimated state covariance
        
		% The covariance values provided here are NOT correct!
        Q_0 = [0.1,0; 0, 0.01]; % top left is u and bottom right is q
...
%             R = measurement_noise(obj,ids,z); 
        ...
        function add_new_landmarks(obj, ids, z)
            ...
            new_cov(old_len + 1: end, old_len + 1: end) = 100*eye(length(new_lmks) * 2);
            obj.x = new_state;
            obj.P = new_cov;
